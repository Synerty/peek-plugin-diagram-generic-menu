...
class RpcForAgent:
    ...
    def makeHandlers(self):
        ...
        yield self.addGenericDiagramMenu.start(funcSelf=self)
        logger.debug("RPCs started")

    # -------------
    @vortexRPC(peekServerName,
               acceptOnlyFromVortex=peekAgentName, additionalFilt=genericDiagramMenuFilt)
    ...
    @vortexRPC(peekServerName, acceptOnlyFromVortex=peekAgentName,
               additionalFilt=genericDiagramMenuFilt, deferToThread=True)
    def addGenericDiagramMenu(self, genericDiagramMenu: GenericDiagramMenuTuple):
        """ Insert a genericDiagramMenu
        ...
        try:
            session.add(genericDiagramMenu)
